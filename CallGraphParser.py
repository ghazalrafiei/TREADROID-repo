...
class CallGraphParser:
    ...
    def is_naf_only_widget(cls, hop):
        ...
        # D@text=https://duckduckgo.com/?kao=-1&kak=-1&naf=&content-desc=&resource-id=url_edittext&class=android.widget.EditText (onClick)
        # print("cls",cls)
        ...
        for pairs in kv:
            if len(pairs) == 2:
                if pairs[0] not in ['resource-id','text','class','naf','content-desc']:
                    ...
                    # print("pairs0",pairs0)
                    # print("curated_kv[-1]",curated_kv)
                    curated_kv[-1][1]+='&'+pairs0
                else:
                ...
                # curated_kv[-1] [['class', 'android.widget.EditText'], ['https://duckduckgo.com/?kao=-1']]
                pairs1=[]
                pairs1=[pairs[0],pairs0]
                curated_kv.append(pairs1)
            
            else:
                assert False
        # print("curated_kv",curated_kv)
        # curated_kv [['content-desc', ''], ['text', 'https://duckduckgo.com/?kao=-1&kak=-1'], ['resource-id', 'url_edittext'], ['class', 'android.widget.EditText'], ['naf', '']]
